...
export function Login({ onSignIn, onSignInWithGoogle }: Props) {
  ...
  const handleGoogle = async () => {
    ...
    setLoading(true);
    try {
      const { error } = (await onSignInWithGoogle()) as {
        error: { message: string } | null;
      };
      if (error) setError(error.message);
    } catch {
    ...
    <div className="min-h-screen flex items-center justify-center relative overflow-hidden">
      ...
      <div className="relative z-10 glass rounded-2xl shadow-2xl p-8 w-full max-w-sm animate-fade-in">
        ...
        <button
          ...
          disabled={loading}
          className="w-full flex items-center justify-center gap-3 bg-white border border-gray-300 rounded-lg px-4 py-2.5 text-sm font-medium text-gray-700 hover:bg-gray-50 hover:border-gray-400 transition-all disabled:opacity-50 shadow-sm"
        >
